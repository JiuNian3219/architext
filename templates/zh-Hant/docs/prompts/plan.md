...
<step_2_interview>
    ...
    **規則**:
    1. **Convention Inheritance (約定繼承)**: 讀取 `02_tech_stack.md` Section 9 中的專案約定。有約定的維度 → 直接繼承為推薦，來源標註 `專案約定`，禁展開選項表（除非此功能有**明確特殊需求**須偏離約定）。無約定的維度 → 按下方規則處理。
    2. 根據專案特徵標籤（UI/Data/CLI/Lib/API）選擇適用維度，略過不適用的
    3. 對每個適用維度：AI 從參考選項庫中選出**最推薦方案**，寫出推薦理由（1-2 句，結合此功能的具體情況）
    4. 僅當某維度存在**兩個以上合理選項且選擇會顯著影響實作**時，才展開選項表讓使用者選擇
    5. 展開的選項表規則不變：3-5 個選項 + `[Z] 自訂`；說明須描述具體行為；AI+/AI- 須為完整句子，禁寫「無」
    6. **功能上下文化（Critical）**: 須用功能設計中確認的實體名、操作名、業務流程來描述選項，禁照搬泛化描述
    ...
    **參考選項庫**:
    ...
    ---
    ### 維度 3: Data Flow (條件 + 約定繼承)

    **提問條件**: 專案含 [?UI+Data]、[?UI+API] 標籤時適用；純 [?CLI]/[?Lib] 略過。
    **約定繼承**: `02_tech_stack.md` §9 Data Flow 有值 → 自動繼承，不提問。僅當此功能需偏離專案預設（如需 Realtime 而專案預設 Standard Request）時才展開選項表。
    ...
    **參考選項庫**:
    ...
    ---
    ### 維度 4: Error Handling (約定繼承)

    > `02_tech_stack.md` §9 已確立專案級錯誤處理策略 → 自動繼承，不提問。
    > 僅當此功能有**專案約定未涵蓋的特殊異常場景**時，才額外補充（如此功能需 Undo/Redo 但專案約定只有 Fail Fast）。
    ...
    **參考選項庫**（通用，AI 根據專案型別微調措辭）:
    ...
    ---
    ### 維度 5: Access & Scope (條件 + 約定繼承)

    **提問條件**: 專案含 [?Web/API] 標籤時提問權限控制；含 [?Lib] 標籤時提問封裝策略；純 [?CLI] 通常略過。
    **約定繼承**: `02_tech_stack.md` §9 Auth & Access 有值 → 認證**機制**繼承（如 JWT/RBAC），但**權限級別**仍為功能級決策（如某功能 Public vs Owner Only）。

    #### [?Web/API] 權限控制
    > 誰能執行此功能的操作、看到什麼資料。認證機制繼承專案約定，此處僅決定此功能的權限級別。
