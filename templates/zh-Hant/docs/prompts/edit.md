...
<meta>
    ...
    <principles>
      ...
      2.  **Incremental**: 僅追加新的 Task 到 `plan.md`，保留已完成的歷史任務（除非需要回復）。
...
<step_1_load>
    **Role**: 產品經理
    **Action**:
    - 讀取 `[[__DOCS_DIR__]]/features/<ID>_<Slug>/spec.md`。
    - 讀取 `[[__DOCS_DIR__]]/features/<ID>_<Slug>/ui.md`。
    - 讀取 `[[__DOCS_DIR__]]/features/<ID>_<Slug>/plan.md`。
...
<step_2_refine_docs>
    **Role**: 需求分析師 & 設計師
    **Action**:
    - 根據 `[context]` 修改 `spec.md` (邏輯/規則變更)。
    - 根據 `[context]` 修改 `ui.md` (介面/互動變更)。
