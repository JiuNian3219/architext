...
3. 粒度校準（核心原則：**一任務 = 一次 `/archi.plan` 會話 = 一個 `features/<slug>/` 子目錄**）：
    ...
    **粒度上限（定量紅線）**：
    ...
    > 一個 Roadmap Task = 能產出**一個內聚 spec.md + 一個可執行 plan.json** 的最小功能單元。

    | 指標 | 上限 | 超出時的動作 |
    |:---|:---|:---|
    | spec.md Scenario 數 | ≤ 6 個 | 回到 `/archi.scope` 拆分，當前任務過大 |
    | plan.json Phase 數 | ≤ 4 個 | 同上 |

    > 此紅線在分解階段作為預判依據；在 `/archi.plan` 執行中若預估超出，須暫停並提示使用者返回 `/archi.scope` 重新拆分，禁強行塞進單一任務。
...
**Infra 任務粒度原則（與 Feature 相反，傾向合併）**：

Infra 任務不直接承載業務邏輯，AI 執行時上下文單一，不存在「實作域過寬」問題。過度拆分 Infra 只會增加依賴鏈複雜度，無益。

| 訊號 | 動作 |
|:---|:---|
| 同屬工程配置、同期執行、技術棧相關 | 合併（如腳手架 + CI + 路由骨架 → 一個 INF 任務） |
| 技術棧完全不同 且 執行時機明顯不同 | 拆分（如 Dexie.js 儲存層 vs Shadcn 主題配置） |
| 某 Infra 產出物被 ≥2 個 Feature 直接呼叫（介面型） | 獨立成任務（須聲明導出介面契約） |
