...
> **Scope:** `[?UI]` 仅适用于有 UI/界面的项目（Web/小程序/桌面应用等）。CLI/API/库项目可忽略。
> **Status:** [Drafting]
...
2.  **Sync**: 随后更新项目的样式配置文件以匹配此定义（依据 `02_tech_stack.md` 技术选型：如 Tailwind 则 `tailwind.config.*`，如 CSS 变量则 `*.css` / `theme.*`，其他栈类推）。
