...
<step_3_generate>
    **Action**: 调用 Skill 执行实际生成。
    ...
    [[SKILL: archi-ui-wireframe|按 skill 的协议，基于 ui_context.md 屏幕索引生成多文件 UI 概念设计。全量模式生成所有屏幕；增量模式仅生成新增/变更屏幕。生成后同步更新 ui_context.md 的屏幕结构摘要。]][[NO-SKILL: （Skill 未安装：请阅读 `[[__DOCS_DIR__]]/skills/archi-ui-wireframe/SKILL.md` 并遵循其协议执行）]]
