<protocol_code>
  ...
  **Goal**: 基于 `features/<id>_<Slug>/plan.md` 任务清单，完成功能开发；遵循 `02_tech_stack.md`（[?UI] 同时遵循 `03_design_tokens.md`）；通过构建、类型、Lint、格式化、测试与审计。
...
<meta>
    ...
    <principles>
      ...
      7.  **Plan Completion Gate**: 结束前验证 plan.md 完成度。AI 可完成的任务须全部完成，仅豁免「人工介入」和「不可抗力」类。
    </principles>
</meta>
...
<step_1_resolve>
    **Role**: 系统分析师
    **Action**:
    1.  **Resolve ID**: 从 `[[__DOCS_DIR__]]/global/00_roadmap.md` 解析 `<id>` → Feature Name、Slug、阶段/状态。
    ...
    3.  **Load Context** (用 Roadmap `📁 Slug` 定位):
        ...
        - `[[__DOCS_DIR__]]/features/<id>_<Slug>/plan.md` — 任务拆解
...
<step_4_validate>
    **Role**: 验证工程师
    **Action** (按序执行，失败须修复；命令以 `02_tech_stack.md` 或项目配置为准):
    ...
    **4.2 Test**
    ...
    | 检查项 | 要求 |
    |:---|:---|
    | **Existing Tests** | 运行已有测试套件，全部通过；禁因新代码破坏旧测试 |
    | **New Coverage** | 为新增/修改的关键逻辑补充测试；纯样式调整可豁免 |

    **4.3 Runtime Verification (硬性要求)**

    > 禁仅通过代码审查或测试就标记完成。
    > 如 `scripts/dev-check` 存在，须先执行；否则按 `02_tech_stack.md` Runtime Verification 手动验证。

    | 项目类型 | 验证动作 | 通过标准 |
    |:---|:---|:---|
    | [?Web] | 启动 dev server → 浏览器操作目标功能路径 | 渲染正常，交互无报错，控制台无异常 |
    | [?API] | 启动服务 → 调用新增/修改的 endpoint | 状态码与 Body 符合 spec |
    | [?CLI] | 执行目标命令（含正常参数 + 边界参数） | stdout 符合预期，exit code 正确 |
    | [?Lib] | 运行示例代码或 playground 验证导出 API | 无运行时错误，返回值正确 |
    | [?Mobile] | 启动模拟器/真机 → 操作目标功能 | 界面正常，交互响应 |
    | [?Desktop] | 启动应用 → 操作目标功能 | 窗口正常，功能可用 |

    **Evidence**: Output 须附验证结果（命令输出摘要 / 截图 / 错误日志）。验证失败须修复后重验。

    **Output**: 每项检查 ✅/❌ 状态与原因；Runtime 验证证据。
</step_4_validate>
...
<step_5_audit>
    ...
    **Checklist**:
    ...
    3.  [?Data] **Data Integrity**: 符合 `04_data_snapshot.md`；字段名/类型一致。
...
<step_6_signoff>
    ...
    **Plan Completion Gate (强制)**:
    1.  运行 `npx archi plan <ID>` 检查 plan.md 任务完成度。
    2.  通过条件: 全部 `[x]` 或未完成项仅属于 🧑人工介入 / 🌐不可抗力。
    3.  未通过: 禁签收，回到 step_3 继续实施。
    4.  豁免项须标注原因和类别。

    **Signoff Action** (Gate 通过后，按序执行):
    1. **CLI 必执行** (须实际在终端执行，禁跳过):
       - `npx archi task <ID> --status done`（或 `active`）。禁直接编辑 roadmap。
       - `npx archi task --check`；失败须修复。
    2. 输出完成任务清单与补丁链接(Code Reference)。
    3. 更新 plan.md，勾选已完成 Checkbox。
