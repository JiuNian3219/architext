...
<step_4_validate>
    **Role**: 验证工程师
    **Action** (按顺序执行，任何失败须修复；命令以 `02_tech_stack.md` 或项目配置为准):
    ...
    | **Test** | 单元/集成测试通过；E2E 仅在关键路径执行(如配置) |
    | **Runtime** | [?Web] 本地预览 [?CLI] 关键命令验证 [?API] 健康检查 [?Lib] 示例验证 |

    任何验证失败不得标记完成；须回滚或修复至通过。

    **Output**: 每项检查的通过/失败状态与原因。
</step_4_validate>
...
<step_6_signoff>
    ...
    **Signoff Action** (Gate 通过后):
    1. 输出完成任务清单与补丁链接(Code Reference)。
    2. 更新 plan.md，勾选已完成 Checkbox。
    3. 运行 `npx archi task <ID> --status done`（或 `active`）更新状态。禁直接编辑 roadmap。
    4. 运行 `npx archi task --check` 验证一致性；失败须修复。
