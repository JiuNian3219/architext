...
<meta>
    ...
    <principles>
      1.  **Spec Immutable**: 严禁修改 `spec.md` 或 `ui.md`（除非 Bug 本身是文档错误）。
...
<step_1_diagnose>
    ...
    **Action**:
    1.  **Resolve Target**:
        - 若提供 `<id>`: 锁定目标 `features/<ID>_<Slug>/`。
        - 若未提供 `<id>`: 分析 `[context]`，在 `features/` 目录下搜索最相关的功能模块。
          - ➡️ 找到唯一匹配: 自动锁定。
          - ➡️ 找到多个匹配: 列出候选列表并询问用户。
          - ➡️ 无法定位: 报错并请求用户指定 ID。
    2.  读取目标目录下的所有文档 (`spec.md`, `ui.md`, `plan.md`) 与相关代码。
