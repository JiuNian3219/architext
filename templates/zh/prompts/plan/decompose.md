...
**重构类粒度例外**：
- 若需求目标是“拆分已有文件 / 重组目录 / 模块边界调整 / 代码太集中 / CSS 模块化 / server 拆分”，它仍是新任务，但不要按文件、组件、目录逐个拆 roadmap。
- decompose 只负责在 roadmap 上追加经过评估后的任务；实现步骤下沉到后续 `/archi.plan <ID>` detail 的 `plan.json`。
- 重构类任务以“独立交付价值 + 独立验证边界 + 风险隔离”为拆分依据；先评估再决定数量，不使用固定数字规则。
- 小型结构整理应倾向合并成一个 roadmap task；只有前端/后端/样式/数据迁移等部分存在清晰的独立验收、独立风险或独立发布顺序时，才拆成多个 roadmap task。
- 同一现有大文件内的组件、常量、工具函数、样式拆分通常合并为一个前端结构重构任务；后端路由/service/utils 拆分通常合并为一个后端结构重构任务；样式只在风险独立或迁移量较大时单独成任务。
- 禁止输出“建立目录结构”“提取常量”“提取工具函数”“拆分某个组件”这类纯步骤型 roadmap task；这些应写进后续 detail 的 plan.json，而不是 roadmap。
- 示例：`main.jsx + server.js + styles.css 太集中，需要拆分` → 先评估实际规模；若只是整理少量文件，可能一个 `代码结构拆分` 任务即可；若前后端风险和验收明显独立，可拆成 `前端结构重构`、`后端结构重构`；样式只有在独立风险明显时才单独成任务。
