<protocol_review_map>
**Trigger**: 路由器 prompts/review.md 分发，或 /archi.review map 别名直接调用
...
<meta>
  ...
  <principles>
    Scan vs Map: 以实际文件系统为 Ground Truth，map.json 为旧快照。
    Smart Granularity: 默认目录级；单文件承载多职责时须细化到文件级。
    Architecture Inference: 新条目的层级归类须参考现有 map 模式 + tech_stack.md。
    Batch Confirm: 所有变更一次性展示，用户批量确认。
    No Re-routing: 如扫描发现项目结构整体异常（大量孤儿目录 / 大批未注册模块）→ 提示用户跑 /archi.review（项目级体检）；禁子协议自动切换。
    IDE-Native First: 利用 IDE 原生能力驱动执行节奏。
  </principles>
</meta>

<step_1_scan>
1. 读取 map.json — 当前架构地图。
2. 读取 tech_stack.md — 目录结构约定、架构模式。
3. 扫描项目目录结构。
   - 排除：.git/、node_modules/、dist/、build/、`[[__DOCS_DIR__]]/`、及 .gitignore 中声明的路径。
   - 深度：跟随 map.json 现有条目的粒度模式。

输出（内部）：实际目录树 + 现有 map 结构。
</step_1_scan>

<step_2_diff>
将实际目录树与 map.json 逐条比对，归类为三种差异。
...
□ 疑似重命名已明确处理（非静默忽略）
□ featureRelations 已同步更新（如适用）
□ lastUpdated 已更新
□ Terminal Gate — task --check + render 通过

输出更新摘要：移除 N 条 / 新增 N 条（含 M 条文件级）/ 重命名 N 条 / 当前总条目数。
</step_5_apply>
