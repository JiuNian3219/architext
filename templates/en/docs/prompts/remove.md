<protocol_remove>
  ...
  **Goal**: Fully decommission the specified Feature — delete docs, code, and global references with zero residue.

<meta>
    <style>Surgical, Cautious, Thorough</style>
    <language>English</language>
    <principles>
      1.  **Dependency Safety**: If other tasks depend on this Feature, dependencies must be resolved before proceeding.
      ...
      4.  **No Side Effects**: Only remove content related to the target Feature; forbidden to touch unrelated files.
    </principles>
</meta>
...
<step_1_resolve>
    **Role**: System Analyst
    **Action**:
    1.  **Resolve ID**: Parse `<id>` from `[[__DOCS_DIR__]]/global/roadmap.json` → Feature Name, Slug, status.
    2.  **ID Not Found** → Error with list of available task IDs.
    3.  **Load Context**:
        - `[[__DOCS_DIR__]]/features/<id>_<Slug>/` — All docs (spec.md, ui.md, plan.json, etc.)
    ...
    **Output**: Target Feature info (ID, name, status, associated file count).
</step_1_resolve>
...
<step_2_impact>
    ...
    **Action**: Scan each layer to produce an impact manifest.
    ...
    | Has downstream deps with `active`/`done` status | **Block** — these tasks may use this Feature's interfaces/components. Must `/archi.edit` to decouple first |

    ### 2.2 Code File Identification

    Locate code files belonging to this Feature via:
    1.  `99_context_glue.md` code paths associated with `<id>`.
    2.  `map.json` modules/directories registered for this Feature.
    3.  `plan.json` file paths mentioned in tasks.
    4.  Scan code directories for files named with Slug or clearly scoped to this Feature.
    ...
    **Docs to delete**:
    - [[__DOCS_DIR__]]/features/<id>_<slug>/  (N files)
    ...
    **Gate**: Proceed to step_3 after user replies OK. Re-warn if `[Breaking]` cross-Feature refs exist.
</step_2_impact>
...
<step_5_summary>
    **Output**: Decommission summary:
    ...
    - **[?present] Manual follow-up needed**: terms/error codes/cross-Feature ref residue
