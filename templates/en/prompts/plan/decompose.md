...
**Boundary**: This protocol may only analyze scope, output a split proposal, and append to `global/roadmap.json` after user OK. It must not create `tasks/<ID>_<Slug>/`, `spec.md`, `plan.json`, `ui.md`, or `design.md`; those are only generated by `/archi.plan <existing ID>` detail.
