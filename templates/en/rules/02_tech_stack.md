...
- `ui_concept.html` is generated by the `archi-ui-wireframe` Skill, covering all user-visible screens
- `ui.md` only declares "which screens/states in ui_concept.html this task is responsible for"; do not redefine global layout
- Code phase uses `ui.md` + `ui_concept.html` as source; implement equivalent visual effect with the **project's own tech stack**
