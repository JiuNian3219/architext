...
|:---|:---|:---|:---|:---|
| `roadmap.json` | `[[__DOCS_DIR__]]/global/roadmap.json` | Task progress DAG: ID/title/status(pending/active/done/blocked)/phase/deps/description | During semantic understanding to match intent to task; Determine task status and deps; Check ID validity | Created by start; Appended by scope; Update status after code/fix completes | Structure: flat `tasks[]` + `nfr[]`, each task must have id/phase/title/status; deps reference must exist; slug in Snake_Case |
