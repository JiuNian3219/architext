...
**Action**: Incorporate feedback, update only the affected parts of `ui_concept.html` (no full rewrite), re-display summary, await confirmation.
...
4. **Output**:
   ...
   - Output summary:
     ...
     **Applied visual spec**:
     ...
     > `/archi.plan <ID>` will read this file to determine each task's UI scope.
     ```
...
3. Output change summary: `MODIFIED: ui_concept.html S-XX (incremental re-color, [N] new states/regions)`
